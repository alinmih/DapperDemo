...
namespace Runner
{
    class Program
    {
        ...
        static void xMain(string[] args)
        {
            ...
            //List_should_produce_correct_results();
        ...
        {
            // arrange
            var respository = CreateRepositoryEx();

            // act
            var contacts = await respository.GetAllAsync();
        ...
        static void Get_all_should_return_6_results_with_addresses()
        {
            ...
            // act
            var contacts = respository.GetAllContactWithAddresses();
        ...
        static void GetIllinoisAddress()
        {
            // arrange
        ...
        static void BulK_insert_should_insert_4_rows()
        {
            ...
            {
                new Contact{FirstName="John", LastName="Barkley"},
                new Contact{FirstName="Tim", LastName="Cook"},
                new Contact{FirstName="Scott", LastName="Martin"},
                new Contact{FirstName="Allen", LastName="Ewing"}
            };
            // act
            var affectedRows = repository.BulkInsertContacts(contacts);

            // assert
            Console.WriteLine($"Rows inserted: {affectedRows}");
            Debug.Assert(affectedRows == 4);

        }

        static void Dynamic_should_produce_correct_results()
        {
            // arrange
            ContactRepositoryEx repositoryEx = CreateRepositoryEx();

            // act
            var contacts = repositoryEx.GetDynamicContactsById(1, 2, 4);
        ...
        static void List_should_produce_correct_results()
        {
            // arrange
            ContactRepositoryEx repositoryEx = CreateRepositoryEx();
            
            // act
            var contacts = repositoryEx.GetContactsById(1, 2, 4);
        ...
        static void Delete_should_remove_entity(int id)
        {
            ...
            var deletedContact = repository2.Find(id);

            // assert
            Debug.Assert(deletedContact == null);
        ...
        static void Modify_should_update_existing_entity(int id)
        {
            ...
            Debug.Assert(modifiedContact.FirstName == "Joe Updated");
        ...
        static void Find_should_retrieve_existing_entity(int id)
        {
            ...
            // assert
            Console.WriteLine("*** Getting Contact ***");
            contact.Output();
            Debug.Assert(contact.FirstName == "Joe");
            Debug.Assert(contact.LastName == "Black");
